...
<div class="card main-card shadow mb-4">
    ...
    <div class="card-header  d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-primary">{{__('Permissions Management')}}</h6>
        <div class="card-action">
            <a class="action-item  btn btn-primary btn-sm" href="{{ dashboard_route('hr.permissions.create') }}"> <i
                ...
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        <a class="btn btn-danger btn-sm" href=""
                           onclick="event.preventDefault(); document.getElementById('delete_form{{$permission->id}}').submit();">
                            <i class="fas fa-trash fa-fw"></i></a>
                        <a class="btn btn-success btn-sm" href="{{ dashboard_route('hr.permissions.edit' , $permission->id) }}"><i class="fas fa-edit fa-fw"></i></a>
                        <a class="btn btn-info btn-sm" href="{{ dashboard_route('hr.permissions.show' , $permission->id) }}"><i class="fas fa-eye fa-fw"></i></a>
                        <form id="delete_form{{$permission->id}}" action="{{ dashboard_route('hr.permissions.destroy' , $permission->id) }}" method="post">
                            @csrf
                            {{method_field('DELETE')}}
                        </form>
                        @else
...
@section('content_js')
    ...
    <script type="text/javascript">
        ...
        $('#users_table').DataTable({
            ...
            dom: "Blfrtip",
            buttons: [
                {extend: 'print', title: 'Permissions Liste ' + date, exportOptions: {columns: [0,1,2,3]}},
                {extend: 'copy', title: 'Permissions Liste ' + date, exportOptions: {columns: [0,1,2,3],}},
                {extend: 'pdf', title: 'Permissions Liste ' + date, exportOptions: {columns: [0,1,2,3],}},
                {extend: 'excel', title: 'Permissions Liste ' + date, exportOptions: {columns: [0,1,2,3],}},
                {extend: 'csv', title: 'Permissions Liste ' + date, exportOptions: {columns: [0,1,2,3],}},
                {extend: 'colvis', title: 'Permissions Liste ' + date, exportOptions: {columns: [0,1,2,3],}},
            ],
